// Shared visual primitives for the paper-analysis figures.
// The colours are sampled from Matplotlib's coolwarm colormap so the browser
// figures use the same palette as the manuscript-generation script.
export const COOLWARM_LUT = [
...
export function coolwarmColor(value) {
  const normalized = Math.max(0, Math.min(1, Number(value) || 0));
  const scaled = normalized * (COOLWARM_LUT.length - 1);
  const index = Math.min(COOLWARM_LUT.length - 2, Math.floor(scaled));
  const fraction = scaled - index;
  const start = hexToRgb(COOLWARM_LUT[index]);
  const end = hexToRgb(COOLWARM_LUT[index + 1]);
  const channels = start.map((channel, channelIndex) => Math.round(channel + (end[channelIndex] - channel) * fraction));
  return `rgb(${channels.join(",")})`;
}

export function coolwarmSample(value) {
  return coolwarmColor(value);
}
